...
namespace ISSUDemoCrypto
{
    public class ComBase
    {
        ...
        public int Read(out byte[] Readbuffer, int NumBytes)
        {
            byte[] BufBytes;
            ...
            if (hComm != INVALID_HANDLE_VALUE)
            {
                ...
                Array.Copy(BufBytes, Readbuffer, BytesRead);
                //m_Wnd.WriteBytes(Readbuffer, false);
                return BytesRead;
            }
            else
            {
        ...
        public int Write(byte[] WriteBytes, int writelen)
        {
            
            if (hComm != INVALID_HANDLE_VALUE)
            {
                //m_Wnd.WriteBytes(WriteBytes,true);
                OVERLAPPED ovlCommPort = new OVERLAPPED();
                int BytesWritten = 0;
                WriteFile(hComm, WriteBytes, writelen,
